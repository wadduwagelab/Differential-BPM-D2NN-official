#!/usr/bin/env python
"""Classification sweep over refractive index (default seeds 0, 1, 2).

For every (seed, n) the script produces

  thin->thin                 thin-layer training and evaluation (independent of n)
  thin->vol (entrance)       thin-trained heights, slab entrance at nominal plane
  thin->vol (centroid)       slab centroid at nominal plane
  thin->vol + delta_z        centroid + one global axial offset fitted on the validation split
  thin->vol + s              centroid + one global height-scale factor fitted on the validation split
  thin->vol + s + delta_z    both corrections (coordinate descent)
  thin + k epochs dBPM fine-tuning -> vol  (k = 1..ft_epochs)
  dBPM->vol                  dBPM training from scratch (centroid convention)
  dBPM->thin                 dBPM-trained heights evaluated with the thin model

All volumetric test numbers are reported both at the training discretisation
(N_sub = n_sub) and at a converged discretisation dz <= lambda / eval_dz_lambda.
"""
import argparse, json, math, os, sys, time, random
import numpy as np
import torch

sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
from dbpm_d2nn.model import D2NN, save_checkpoint
from dbpm_d2nn.data import classification_datasets, make_loaders, detector_patches_from_geometry
from dbpm_d2nn.evaluate import evaluate_classification
from dbpm_d2nn.train import train_classification
from dbpm_d2nn.calibrate import fit_axial_shift, fit_height_scale
from dbpm_d2nn.metrics import fabrication_statistics

p = argparse.ArgumentParser()
p.add_argument("--dataset", default="mnist", choices=["mnist", "fmnist"])
p.add_argument("--encoding", default="phase", choices=["phase", "amplitude"])
p.add_argument("--n_values", default="1.1,1.2,1.3,1.4,1.5,1.7,1.9")
p.add_argument("--seeds", default="0,1,2")
p.add_argument("--grid", type=int, default=200); p.add_argument("--digit", type=int, default=112)
p.add_argument("--wavelength", type=float, default=532e-9)
p.add_argument("--z_lam", type=float, default=40.0); p.add_argument("--aperture_lam", type=float, default=107.0)
p.add_argument("--L", type=int, default=5)
p.add_argument("--epochs", type=int, default=10); p.add_argument("--ft_epochs", type=int, default=5)
p.add_argument("--batch", type=int, default=128); p.add_argument("--lr", type=float, default=1e-3)
p.add_argument("--n_sub", type=int, default=15, help="minimum number of sub-slices used for training")
p.add_argument("--train_dz_lambda", type=float, default=3.0,
               help="training discretisation: N_sub = max(n_sub, ceil(this * h_max / lambda)), i.e. dz <= lambda/this")
p.add_argument("--eval_dz_lambda", type=float, default=12.0, help="converged evaluation: dz <= lambda/this")
p.add_argument("--fit_val_samples", type=int, default=2000, help="validation images used by the calibration fits")
p.add_argument("--skip_bpm", action="store_true"); p.add_argument("--skip_fits", action="store_true")
p.add_argument("--skip_ft", action="store_true")
p.add_argument("--out_dir", required=True)
p.add_argument("--device", default="cuda:0"); p.add_argument("--workers", type=int, default=4)
args = p.parse_args()

dev = args.device
os.makedirs(args.out_dir, exist_ok=True)
ckpt_dir = os.path.join(args.out_dir, "checkpoints"); os.makedirs(ckpt_dir, exist_ok=True)
lam = args.wavelength
dx = args.aperture_lam * lam / args.grid
z = args.z_lam * lam
logf = open(os.path.join(args.out_dir, "log.txt"), "a")


def log(s):
    print(s, flush=True); logf.write(s + "\n"); logf.flush()


def seed_all(seed):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed); torch.cuda.manual_seed_all(seed)


train, val, test = classification_datasets(args.dataset, args.grid, args.digit)
patches = detector_patches_from_geometry(args.grid, args.grid, dx, lam)
log(f"== {args.dataset}/{args.encoding}: grid {args.grid}, dx={dx/lam:.3f}lam, z={args.z_lam}lam, patches {patches[0]}")
common = dict(H=args.grid, W=args.grid, L=args.L, wavelength=lam, dx=dx, z=z, z_output=z, phase_param="wrap")


def train_n_sub(n):
    """N_sub used for training / fine-tuning at index n (dz <= lambda/train_dz_lambda, at least n_sub)."""
    return max(args.n_sub, int(math.ceil(args.train_dz_lambda / (n - 1.0) - 1e-9)))


def eval_n_sub(n):
    return max(args.n_sub, int(math.ceil(args.eval_dz_lambda / (n - 1.0) - 1e-9)))


def acc_of(model, loader):
    return evaluate_classification(model, loader, patches, args.encoding, dev)["acc"]


def vol_eval(design: D2NN, n, *, mode="anchor", anchor=0.5, shift=0.0, scale=1.0, loader=None):
    """Evaluate the design's heights with the volumetric model at training and converged N_sub."""
    out = {}
    for tag, ns in (("train", train_n_sub(n)), ("fine", eval_n_sub(n))):
        m = D2NN(**common, n_material=n, n_sub=ns, axial_mode=mode, anchor=anchor, axial_shift=shift,
                 height_scale=scale).to(dev)
        m.load_layers_from(design)
        out[tag] = acc_of(m, loader)
        out[f"{tag}_n_sub"] = m.n_sub
    return out


results_path = os.path.join(args.out_dir, "results.json")
all_results = json.load(open(results_path)) if os.path.exists(results_path) else []
done = {(r["seed"], r["n"]) for r in all_results}

for seed in [int(s) for s in args.seeds.split(",")]:
    seed_all(seed)
    tl, vl, te = make_loaders(train, val, test, batch_size=args.batch, seed=seed, num_workers=args.workers, test_batch=250)
    # fixed 2000-image subset of the validation split used for the one-parameter calibration fits (speed)
    from torch.utils.data import DataLoader, Subset
    vl_fit = DataLoader(Subset(val, list(range(min(args.fit_val_samples, len(val))))), batch_size=250, shuffle=False,
                        num_workers=args.workers, pin_memory=True)
    # ---------------------------------------------------------------- thin model (independent of n)
    thin_path = os.path.join(ckpt_dir, f"thin_seed{seed}.pt")
    thin = D2NN(**common, n_material=1.5, n_sub=1, seed=seed).to(dev)
    if os.path.exists(thin_path):
        thin.load_state_dict(torch.load(thin_path, map_location=dev)["state_dict"]); thin.eval()
        log(f"[seed {seed}] loaded thin model")
    else:
        log(f"[seed {seed}] training thin model")
        train_classification(thin, tl, patches, args.encoding, dev, epochs=args.epochs, lr=args.lr,
                             epoch_callback=lambda ep, r: {"val_acc": acc_of(thin, vl)}, log=log)
        save_checkpoint(thin, thin_path, seed=seed, kind="thin", dataset=args.dataset, encoding=args.encoding)
    acc_thin_thin = acc_of(thin, te); acc_thin_val = acc_of(thin, vl)
    log(f"[seed {seed}] thin->thin test acc {acc_thin_thin:.2f} (val {acc_thin_val:.2f})")

    for n in [float(v) for v in args.n_values.split(",")]:
        if (seed, n) in done:
            log(f"[seed {seed} n={n}] already done"); continue
        t_start = time.time()
        rec = {"seed": seed, "n": n, "dataset": args.dataset, "encoding": args.encoding,
               "acc_thin_thin": acc_thin_thin, "val_acc_thin_thin": acc_thin_val,
               "h_max_over_lambda": 1.0 / (n - 1.0), "T_over_z": 1.0 / ((n - 1.0) * args.z_lam),
               "eval_n_sub": eval_n_sub(n), "train_n_sub": train_n_sub(n)}
        # ------------------------------------------------------------ thin -> volumetric (raw)
        rec["thin_vol_entrance"] = vol_eval(thin, n, mode="legacy", loader=te)
        rec["thin_vol_centroid"] = vol_eval(thin, n, mode="anchor", anchor=0.5, loader=te)
        rec["thin_vol_exit"] = vol_eval(thin, n, mode="anchor", anchor=1.0, loader=te)
        rec["thin_vol_entrance_anchored"] = vol_eval(thin, n, mode="anchor", anchor=0.0, loader=te)
        log(f"[seed {seed} n={n}] thin->vol entrance(legacy) {rec['thin_vol_entrance']} | centroid {rec['thin_vol_centroid']}")
        # ------------------------------------------------------------ one-parameter calibrations (validation split)
        base = D2NN(**common, n_material=n, n_sub=train_n_sub(n), axial_mode="anchor", anchor=0.5).to(dev)
        base.load_layers_from(thin)
        if not args.skip_fits:
            obj = lambda m: -acc_of(m, vl_fit)
            t0 = time.time()
            fdz = fit_axial_shift(base, obj)
            rec["fit_dz"] = {"axial_shift_m": fdz["axial_shift"], "axial_shift_over_lambda": fdz["axial_shift"] / lam,
                             "axial_shift_over_T": fdz["axial_shift"] / fdz["T"], "val_acc": -fdz["objective"],
                             "grid": [(v / lam, -o) for v, o in fdz["grid"]]}
            rec["fit_dz"].update({"test_" + k: v for k, v in vol_eval(thin, n, shift=fdz["axial_shift"], loader=te).items()})
            fs = fit_height_scale(base, obj)
            rec["fit_s"] = {"height_scale": fs["height_scale"], "val_acc": -fs["objective"],
                            "grid": [(v, -o) for v, o in fs["grid"]]}
            rec["fit_s"].update({"test_" + k: v for k, v in vol_eval(thin, n, scale=fs["height_scale"], loader=te).items()})
            # joint: shift given s*, then s given shift*
            b2 = base.clone(height_scale=fs["height_scale"])
            fj1 = fit_axial_shift(b2, obj, height_scale=fs["height_scale"])
            b3 = base.clone(axial_shift=fj1["axial_shift"])
            fj2 = fit_height_scale(b3, obj, axial_shift=fj1["axial_shift"])
            s_j, dz_j = fj2["height_scale"], fj1["axial_shift"]
            rec["fit_joint"] = {"height_scale": s_j, "axial_shift_m": dz_j, "axial_shift_over_lambda": dz_j / lam,
                                "val_acc": -fj2["objective"]}
            rec["fit_joint"].update({"test_" + k: v for k, v in vol_eval(thin, n, shift=dz_j, scale=s_j, loader=te).items()})
            log(f"[seed {seed} n={n}] fits ({time.time()-t0:.0f}s): dz*={rec['fit_dz']['axial_shift_over_lambda']:+.3f}lam -> "
                f"test {rec['fit_dz']['test_train']:.2f}/{rec['fit_dz']['test_fine']:.2f} | s*={fs['height_scale']:.3f} -> "
                f"{rec['fit_s']['test_train']:.2f}/{rec['fit_s']['test_fine']:.2f} | joint s={s_j:.3f} dz={dz_j/lam:+.3f}lam -> "
                f"{rec['fit_joint']['test_train']:.2f}/{rec['fit_joint']['test_fine']:.2f}")
        # ------------------------------------------------------------ volumetric fine-tuning from the thin solution
        if not args.skip_ft:
            ft = D2NN(**common, n_material=n, n_sub=train_n_sub(n), axial_mode="anchor", anchor=0.5).to(dev)
            ft.load_layers_from(thin)
            hist = train_classification(ft, tl, patches, args.encoding, dev, epochs=args.ft_epochs, lr=args.lr, anneal=False,
                                        epoch_callback=lambda ep, r: {"test_acc": acc_of(ft, te), "val_acc": acc_of(ft, vl)}, log=log)
            rec["finetune"] = {"epochs": [h["epoch"] for h in hist], "test_acc": [h["test_acc"] for h in hist],
                               "val_acc": [h["val_acc"] for h in hist], "train_loss": [h["train_loss"] for h in hist],
                               "final_fine": vol_eval(ft, n, loader=te)["fine"]}
            save_checkpoint(ft, os.path.join(ckpt_dir, f"ft_seed{seed}_n{n:.2f}.pt"), seed=seed, n=n, kind="thin_finetuned")
            log(f"[seed {seed} n={n}] fine-tune test acc per epoch: {['%.2f' % a for a in rec['finetune']['test_acc']]}")
        # ------------------------------------------------------------ dBPM from scratch
        if not args.skip_bpm:
            seed_all(seed)
            bpm = D2NN(**common, n_material=n, n_sub=train_n_sub(n), axial_mode="anchor", anchor=0.5, seed=seed).to(dev)
            hist = train_classification(bpm, tl, patches, args.encoding, dev, epochs=args.epochs, lr=args.lr, anneal=True,
                                        epoch_callback=lambda ep, r: {"val_acc": acc_of(bpm, vl)}, log=log)
            rec["bpm_vol"] = vol_eval(bpm, n, loader=te)
            rec["bpm_vol"]["val_acc"] = acc_of(bpm, vl)
            rec["bpm_vol"]["train_loss"] = [h["train_loss"] for h in hist]
            thin_of_bpm = D2NN(**common, n_material=n, n_sub=1).to(dev); thin_of_bpm.load_layers_from(bpm)
            rec["bpm_thin"] = acc_of(thin_of_bpm, te)
            save_checkpoint(bpm, os.path.join(ckpt_dir, f"bpm_seed{seed}_n{n:.2f}.pt"), seed=seed, n=n, kind="bpm")
            log(f"[seed {seed} n={n}] dBPM->vol {rec['bpm_vol']} | dBPM->thin {rec['bpm_thin']:.2f}")
            # fabrication statistics of the height maps
            h_max = bpm.h_max
            rec["fab_bpm"] = [fabrication_statistics(h.detach().cpu().numpy(), dx, lam, h_max) for h in bpm.heights()]
        hthin = D2NN(**common, n_material=n, n_sub=1).to(dev); hthin.load_layers_from(thin)
        rec["fab_thin"] = [fabrication_statistics(h.detach().cpu().numpy(), dx, lam, hthin.h_max) for h in hthin.heights()]
        rec["time_s"] = time.time() - t_start
        all_results.append(rec)
        with open(results_path, "w") as f:
            json.dump(all_results, f, indent=1)
        log(f"[seed {seed} n={n}] done in {rec['time_s']/60:.1f} min")
log("sweep finished")
